...
class SlackDataLoader:
    ...
    def get_channel_messages(self, channel_name):
        ...
        Returns:
            list: List of messages from the specified channel.
        '''
        channel_messages = []
        
        # Assuming channel_name is in self.channels
        channel_id = next((channel['id'] for channel in self.channels if channel['name'] == channel_name), None)
        
        if channel_id:
            # Look for messages in the channel
            channel_path = os.path.join(self.path, channel_id)
            
            for json_file in glob.glob(f"{channel_path}/*.json"):
                with open(json_file, 'r', encoding="utf8") as slack_data:
                    file = json.load(slack_data)
                    channel_messages.extend(file)
